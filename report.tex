...
In the process of modeling sewerage networks, the main component is the drained area (catchment) from which water is collected to each conduit (pipe). If the area for a single subcatchment is derived from a mathematical model, we have to create the geometry of that territory. In this paper, we want to make geometric visualization for partitioning of a given living area, with respect to the known water debit, relative to each pipe. If each pipe is represented as edges and these edges form a polygon then our main goal is to make geometric visualization of a polygon area partitioning.  

Despite, this problem is important in hydrology, there is little published information in the hydrological literature. 

In paper \cite{han:bra:1}, two approaches for automated generation of Thiessen polygons are considered. A triangulation method and a grid method are described and the ways for their implementation in hydrological applications is given. Also, procedure of deriving the relationship between the catchment area, grid size and accuracy indicator, based on weighted mean error, is described. 

In paper \cite{mai:mik:1}, web-based urban water management modeling platform for simplifying the setup and use of complex integrated models is presented. The paper demonstrates how flooding behavior of new urban developments can easily be assessed with the web platform, i.e. to inform discussions in interdisciplinary and interactive stakeholder workshops.  Also, in the paper the workﬂow of the used DynaMind model is shown. The inputs and the user deﬁned catchment are presented as polygons and an existing or user deﬁned combined sewer system model which will be affected by the new catchments. This polygon is divided into several smaller polygons/subcatchments by normal distributing new inlet nodes within the polygon and creating a Voronoi diagram on the bases of these nodes. 

Paper \cite{lay:day:1} is focused on the generation of three dimensional models of large urban/suburban environments. Automated tools can be used to provide a rapid coarse model of a city. These techniques can be tailored to generate geometry of appropriate complexity for real-time rendering, thereby negating the need for the simplification stage. In this paper, roof modelling for rectilinear polygons is used, and the procedure to partition  on polygon into rectangles is described.

Paper \cite{lockie:1} outlines the technical capability and functionality of SWMM. The advantages, disadvantages and applicability of SWMM are discussed and a brief comparison between SWMM and other common hydraulic models is presented. The hydraulic engine of SWMM has been proven and tested more than 40 years and it is reported as the most widely applied stormwater model in US and New Zealand.
...
Digital elevation model (DEM) can be used to automatically generate catchment areas. San Francisco Department of Public Work (SF DPW) applied similar techniques at a fine resolution.  These results are used to study pipe hydraulics for San Francisco’s several systems. To automate the drainage catchment delineation process, a computer script in ArcGIS Model Builder called the Urbane Drainage Model is created. The Urbane Drainage Model is based on the commonly used “sleepiest path” approach, using a gridded evaluation model. Catchment areas were delineated for the City of San Francisco. A subcatchment was created for each drain in the area.

Mathematically, the problem is formulated in the following way:
...
The vertices of the polygon are denoted by $v_i$, $ i = 1, \ldots, N$.
...
Standard visual interpretation is given on Fig. \ref{fig:sub4}.

We are looking for the following visual (geometrical) interpretation (Fig. \ref{fig:sub2})

\sect{2.~Solution approaches}

\subsect{2.1.~Monte Carlo Flooding}

With this algorithm the polygon will be divided in $N$ irregular shapes. Let us denote the area of the figures by $c_i, i = 1. \ldots, N$. The following is the pseudocode of the proposed MCFA.
...
\begin{itemize}
  ...
  \item[Step 4.2]  Add nearest points to the boundary figure $IF_i$ in the figure $IF_i$. 
\end{itemize}
\end{itemize}
\noindent\rule{\textwidth}{1pt}
...
First, we choose the number of edges $N$ and $b_i$ randomly.

Each of these points, including the vertices of the polygon will be connected, and on this way we form $N$ triangles denoted by $T_i$. Let us denote the area of the triagles $T_i, i=1, \ldots, N $ by $a_i$. The area of the polygon, outside the triangles is denoted by $W$.  This is shown on Fig. \ref{fig:sub6}.
...
Now, we regard one triangle.  Let $d_i$ be the distance from a random point $t(x_i, y_i) \in T_i$ to the $i$-edge from the polygon. 
...
where $k$ is a total number of the points of the triangle  and

$$D = \sum_{i=1}^N d_i$$.

We need to estimate the cost function
$$cost f = D+W + A + B$$.
...
Our goal is to minimize the cost function. Since, we need to minimize $D, W, A, B$. The set of values for $W$ is $[0, count( p(x_i, y_i))], \forall p(x_i, y_i) \in P$. From this we can conclude that the optimal value for W is 0.
The set of values for $A$ is $[0, count (p(x_i, y_i))], \forall p(x_i, y_i) \in P$ and we can conclude that the minimal value of $A$ is 0. On the other hand, $B$ have set of values $[N,count( p(x_i, y_i))], \forall p(x_i, y_i) \in P$ and therefore its minimal value is $N$, where $N$ is the number of edges of the polygon.
if we succeed to minimize $A, B, W$, we will calculate the minimum value of the cost function.
...
\item[Step 3.2] If the desired area is accomplished, we fix the side of the corresponding trapezoid and stop drawing parallel sides for it.
\end{itemize}
...
Monte Carlo Flooding is very efficient heuristics, but it has few disadvantages concerning the solution of the problem. The biggest disadvantage is related to the fact that the 2D space is divided in shapes similar to semi-circles when polygons are required. A second disadvantage is about the optimal distribution of the sub-areas, which is clearly visible on Fig. \ref{fig:sub7}. 
...
The experimental results from the methods applied for the problem of visualising the partitioning of the geometry show that the Percent Distributed Clustering and Offsetting Polygon method produce much better results than the Monte Carlo Flooding method. Nevertheless, none of these methods is proven to be optimal and the most accurate solution of this problem is still to be found.
